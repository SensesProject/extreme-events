      ...
      <g class="chart" :transform="`translate(0 ${dims.titleHeight})`">
        ...
        <g class="subject" v-for="(subject, i) in subjects" :key="i">
          ...
          <g class="level" v-for="(level, i) in subject.levels" :key="i">
            ...
            <path v-bind="filters.climate" :class="level.color" :d="level.line"/>
            <g class="gradient" v-bind="filters.gradient">
            ...
            <g class="climate" v-for="(climate, i) in level.climates" :key="i">
              ...
              <path v-bind="filters.impact" :class="level.color" :d="climate.line"/>
          ...
          levels: allLevels.map((level, i) => {
            ...
            const climates = Object.keys(data[subject][level].cm).map((climate, i2, climates) => {
              ...
              return {
                ...
                impacts,
                line: `M${impacts.map(impact => `${impact.x + dims.climateWidth / impacts.length / 2},${impact.y}`).join(' L')}`
              }
            }).filter(d => d !== null)
            return {
              val: format(val),
              ...
              climates,
              line: `M${climates.map(impact => `${impact.x + dims.climateWidth / 2},${impact.y}`).join(' L')}`
            }
          })
        }
      })
