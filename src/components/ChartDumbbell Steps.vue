      ...
      <g class="chart" :transform="`translate(0 ${dims.titleHeight})`">
        ...
        <g class="subject" v-for="(subject, i) in subjects" :key="i">
          ...
          <g class="level" v-for="(level, i) in subject.levels" :key="i">
            ...
            <g class="gradient" v-bind="filters.climate">
              <line v-for="(climate, i) in level.climates.filter(c => c.gradient != null)" :key="i" :transform="climate.transform"
                :y2="climate.gradient.height" :class="level.color"/>
            </g>
            <g class="climate" v-for="(climate, i) in level.climates" :key="i">
              ...
              <!-- <g class="gradient" v-if="category !== 'tropical cyclone'" v-bind="filters.impact">
                <rect v-for="(impact, i) in climate.impacts" :key="i" :transform="impact.transform"
                  :height="impact.gradient.height" :width="1" :x="impact.gradient.x - 0.5" :fill="impact.gradient.fill"/>
              </g> -->
  ...
  computed: {
          ...
          levels: allLevels.map((level, i) => {
            ...
            return {
              ...
              climates: climates.map((climate, i2) => {
                ...
                const preVal = i2 === 0 ? null : data[subject][level].cm[climates[i2 - 1]].median
                console.log(preVal)
                const y = yScale(val)
                ...
                return {
                  ...
                  impacts: impacts.map((impact, i3) => {
                    ...
                    return {
                      ...
                      gradient: this.category === 'tropical cyclone' ? null : {
                        height: yScale(preVal) - y,
                        fill: gradients[level],
                        x: impactWidth / 2
                      }
    ...
    filters () {
      ...
      return {
        ...
        climate: view === 'climate' ? { opacity: 1, strokeWidth: 2 } : { opacity: 0.3, strokeWidth: 1 },
        impact: view === 'impact' ? { opacity: 1, strokeWidth: 2 } : { opacity: 0.4, strokeWidth: 1 }
      }
    }
  },
  methods: {
